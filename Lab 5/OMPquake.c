...
#pragma omp for private(my_cpu_id, c0, d1, d2) 
		for (i = 0; i < ARCHnodes; i++) {
			c0[0] = ARCHcoord[i][0];
			c0[1] = ARCHcoord[i][1];
			c0[2] = ARCHcoord[i][2];
	...
	double(*v)[3], double(*w)[3])
{
	int i, j;
	int Anext, Alast, col;
	double sum0, sum1, sum2, v0, v1, v2;
#pragma omp parallel private(my_cpu_id, i, Anext, Alast, col, sum0, sum1, sum2, v0, v1, v2) shared(w1, w2, A, v, w)
	{
...
#pragma omp for
		for (i = 0; i < nodes; i++) {
			...
			while (Anext < Alast) {
				...
				v1 = v[col][0];
				v2 = v[col][0];
				sum0 += A[Anext][0][0] * v0 + A[Anext][0][1] * v1 + A[Anext][0][2] * v2;
				sum1 += A[Anext][1][0] * v0 + A[Anext][1][1] * v1 + A[Anext][1][2] * v2;
				sum2 += A[Anext][2][0] * v0 + A[Anext][2][1] * v1 + A[Anext][2][2] * v2;
				...
				v1 = v[i][1];
				v2 = v[i][2];
				w1[my_cpu_id][col].first += A[Anext][0][0] * v0 + A[Anext][1][0] * v1 + A[Anext][2][0] * v2;
				w1[my_cpu_id][col].second += A[Anext][0][1] * v0 + A[Anext][1][1] * v1 + A[Anext][2][1] * v2;
				w1[my_cpu_id][col].third += A[Anext][0][2] * v0 + A[Anext][1][2] * v1 + A[Anext][2][2] * v2;
				Anext++;
			}
