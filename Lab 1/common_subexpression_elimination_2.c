...
{
	double PSNR = 0, t;
	int i, j, k=SIZE,res1=0,res2=0, a, b, c, d, e, f;
	unsigned int p;
	int res;
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			d=j-1;
			e=j+1;
			res1 += input[b + d] * horiz_operator[0][0];	// i=-1 , j=-1
			res1 += input[b + j] * horiz_operator[0][1];		// i=-1 , j=0
			res1 += input[b + e] * horiz_operator[0][2];	// i=-1 , j=1
			f=i*SIZE;
			res1 += input[f + d] * horiz_operator[1][0];	// i=0 , j=-1
			res1 += input[f + j] * horiz_operator[1][1];		// i=0 , j=0
			res1 += input[f + e] * horiz_operator[1][2];	// i=0 , j=1
			c=(i+1)*SIZE;
			res1 += input[c + d] * horiz_operator[2][0];	// i=1 , j=-1
			res1 += input[c + j] * horiz_operator[2][1];	// i=1 , j=0
			res1 += input[c + e] * horiz_operator[2][2];	// i=1 , j=1
			res2 += input[b + d] * vert_operator[0][0];	// i=-1 , j=-1
			res2 += input[b + j] * vert_operator[0][1];		// i=-1 , j=0
			res2 += input[b + e] * vert_operator[0][2];	// i=-1 , j=1
			res2 += input[f + d] * vert_operator[1][0];	// i=0 , j=-1
			res2 += input[f + j] * vert_operator[1][1];		// i=0 , j=0
			res2 += input[f + e] * vert_operator[1][2];	// i=0 , j=1
			res2 += input[c + d] * vert_operator[2][0];	// i=1 , j=-1
			res2 += input[c + j] * vert_operator[2][1];	// i=1 , j=0
			res2 += input[c + e] * vert_operator[2][2];	// i=1 , j=1
...
// 			p = convolution2D(i, j, input, horiz_operator) * convolution2D(i, j, input, horiz_operator) + convolution2D(i, j, input, vert_operator) * convolution2D(i, j, input, vert_operator);
			...
			if (res > 255)
				...
				output[a] = (unsigned char)res;
			
			t = (output[a] - golden[a])*(output[a] - golden[a]);
			PSNR += t;
