...
// 		}
// 	}
	res += input[(posy -1)*SIZE + posx -1] * operator[0][0];	// i=-1 , j=-1
	res += input[(posy -1)*SIZE + posx] * operator[0][1];		// i=-1 , j=0
	res += input[(posy -1)*SIZE + posx + 1] * operator[0][2];	// i=-1 , j=1
	res += input[(posy)*SIZE + posx - 1] * operator[1][0];	// i=0 , j=-1
	res += input[(posy)*SIZE + posx] * operator[1][1];		// i=0 , j=0
	res += input[(posy)*SIZE + posx + 1] * operator[1][2];	// i=0 , j=1
	res += input[(posy + 1)*SIZE + posx - 1] * operator[2][0];	// i=1 , j=-1
	res += input[(posy + 1)*SIZE + posx] * operator[2][1];	// i=1 , j=0
	res += input[(posy + 1)*SIZE + posx + 1] * operator[2][2];	// i=1 , j=1
	//printf("%d", res);
	return(res);
}
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			 ...
			 * of the derivative.							  */
			res1 += input[(i -1)*SIZE + j -1] * horiz_operator[0][0];	// i=-1 , j=-1
			res1 += input[(i -1)*SIZE + j] * horiz_operator[0][1];		// i=-1 , j=0
			res1 += input[(i -1)*SIZE + j + 1] * horiz_operator[0][2];	// i=-1 , j=1
			res1 += input[(i)*SIZE + j - 1] * horiz_operator[1][0];	// i=0 , j=-1
			res1 += input[(i)*SIZE + j] * horiz_operator[1][1];		// i=0 , j=0
			res1 += input[(i)*SIZE + j + 1] * horiz_operator[1][2];	// i=0 , j=1
			res1 += input[(i + 1)*SIZE + j - 1] * horiz_operator[2][0];	// i=1 , j=-1
			res1 += input[(i + 1)*SIZE + j] * horiz_operator[2][1];	// i=1 , j=0
			res1 += input[(i + 1)*SIZE + j + 1] * horiz_operator[2][2];	// i=1 , j=1
			res2 += input[(i -1)*SIZE + j -1] * vert_operator[0][0];	// i=-1 , j=-1
			res2 += input[(i -1)*SIZE + j] * vert_operator[0][1];		// i=-1 , j=0
			res2 += input[(i -1)*SIZE + j + 1] * vert_operator[0][2];	// i=-1 , j=1
			res2 += input[(i)*SIZE + j - 1] * vert_operator[1][0];	// i=0 , j=-1
			res2 += input[(i)*SIZE + j] * vert_operator[1][1];		// i=0 , j=0
			res2 += input[(i)*SIZE + j + 1] * vert_operator[1][2];	// i=0 , j=1
			res2 += input[(i + 1)*SIZE + j - 1] * vert_operator[2][0];	// i=1 , j=-1
			res2 += input[(i + 1)*SIZE + j] * vert_operator[2][1];	// i=1 , j=0
			res2 += input[(i + 1)*SIZE + j + 1] * vert_operator[2][2];	// i=1 , j=1
